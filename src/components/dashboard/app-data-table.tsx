...
export function DataTable({
  ...
  const requestSort = (key: keyof AppClassEntry) => {
              ...
              {visibleColumns.map((col) => (
                <TableHead key={col.key}>
                  {col.sortable ? (
                    <Button
                      variant="ghost"
                      onClick={() => requestSort(col.key as keyof AppClassEntry)}
                      className="-ml-4"
                    >
                      {col.header}
                ...
                <TableRow key={row.id}>
                  {visibleColumns.map((col) => (
                    <TableCell key={col.key} className="whitespace-nowrap">
                      {String(row[col.key as keyof AppClassEntry] ?? '')}
                    </TableCell>
                  ))}
                </TableRow>
              ))

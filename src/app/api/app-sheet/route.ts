...
export async function POST(request: Request) {
  try {
    ...
    if (!rows || rows.length < 3) { // Need at least header row (2) and one data row
      return NextResponse.json(
        { error: "No data found in the 'APP_Dashboard' sheet. The header is expected on row 2." },
        { status: 404 }
      );
    }
    
    const header = rows[1].map(h => h.trim()); // Header is on the second row
    const classEntryKeys = Object.keys(getInitialAppClassEntry()) as (keyof AppClassEntry)[];
    
    const normalizedKeyMap = new Map(classEntryKeys.map(k => [normalizeHeader(k), k]));

    const specialHeaderMap: {[key: string]: keyof AppClassEntry} = {
        'teacherentrytimet4530': 'teacherEntryTime',
        'slideqact15': 'slideQAC',
        'teacher1': 'teacher',
        'totalattendanceliveclass': 'totalAttendance',
        'classattendance': 'classAttendancePercent',
        'classdurationminutes': 'classDuration',
        'averagewatchtimeminutes': 'averageWatchtime',
        'averagewatchtime': 'averageWatchtimePercent',
        'averagerating': 'averageClassRating',
        'userrated': 'userRatedPercent',
        'uniqueusersrated': 'uniqueUsersRated',
        'usercommented': 'userCommentedPercent',
        'totaldoubt': 'totalDoubt',
        'userdoubted': 'userDoubtedPercent',
        'doubtstarted': 'doubtStartedPercent',
        'doubtresolved': 'doubtResolvedPercent',
        'pollparticipation': 'pollParticipationPercent',
        'avgpollengaged': 'avgPollEngagedPercent',
    ...
    const headerMap: { [key: number]: keyof AppClassEntry } = {};
    header.forEach((h, i) => {
    ...
    const data = rows.slice(2).map((row, index) => { // Data starts from the third row
      const entry: Partial<AppClassEntry> = { id: String(index + 1) };
...
function getInitialAppClassEntry(): AppClassEntry {
    return {
      id: '',
      date: '',
      product: '',
      ...
      userRatedPercent: '',
      uniqueUsersRated: '',
      userCommentedPercent: '',
      totalDoubt: '',
      userDoubtedPercent: '',
      doubtStartedPercent: '',
      doubtResolvedPercent: '',
      pollParticipationPercent: '',
      avgPollEngagedPercent: '',
      issuesType: '',
      timeStamp: '',
      liveClassId: '',
      issueDetails: '',
